...
const AddBookForm = ({ currentId, setCurrentId }) => {
  const [postData, setPostData] = useState({oldPrice: '', price: '', pages: '', title: '', author: '', publisher: '',  description: '', isbns: '', category: '', bookImage: '' });
...
//   }, [post]);
  ...
  const clear = () => {
    setCurrentId(0);
    setPostData({ oldPrice: '', price: '', pages: '', title: '', author: '', publisher: '', description: '', isbns: '', category: '', bookImage: '' });
  };
  ...
  return (
    <Paper className={classes.paper} elevation={4}>
       <Snackbar open={formError.open} autoHideDuration={3000} onClose={() => setFormError({ ...formError, open: false })}  anchorOrigin={{ vertical: 'top', horizontal: 'right' }}>
        ...
        <Typography variant="h4" className={classes.mainHead}> Add New Book Form</Typography>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        ...
        <Grid container spacing={2}>
        <Grid item xs={12} sm={6} md={6}>
        <TextField name="isbns" variant="outlined" label="ISBN (comma separated)" fullWidth value={postData.isbns} onChange={(e) => setPostData({ ...postData, isbns: e.target.value.split(',') })} />
        </Grid>
        <Grid item xs={12} sm={6} md={6}>
        ...
        <Grid container spacing={2}>
        <Grid item xs={12} sm={6} md={6}>
        <TextField name="oldPrice" type="number" variant="outlined" label="Original Price" fullWidth value={postData.oldPrice} onChange={(e) => setPostData({ ...postData, oldPrice: e.target.value })} />
        </Grid>
        <Grid item xs={12} sm={6} md={6}>
        <TextField name="price" type="number" variant="outlined" label="Discounted Price" fullWidth value={postData.price} onChange={(e) => setPostData({ ...postData, price: e.target.value })} />
        </Grid>
        </Grid>
        <Grid container spacing={2}>
        <Grid item xs={12} sm={6} md={6}>
        <TextField name="pages" type="number" variant="outlined" label="Numner of Pages" fullWidth value={postData.pages} onChange={(e) => setPostData({ ...postData, pages: e.target.value })} />
        </Grid>
        <Grid item xs={12} sm={6} md={6}>
        <Select
        ...
        <div className={classes.fileInput}><FileBase type="file" multiple={false} onDone={({ base64 }) => setPostData({ ...postData, bookImage: base64 })} /></div>
